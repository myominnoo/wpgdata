...
| `peg_query()` | Filter, select, and sort |
| `peg_all()` | Download all rows |
```
...
Before working with any dataset, use `peg_catalogue()` to browse all
available datasets on the portal. This is the best starting point when
you don't yet know a dataset ID:
```{r peg_catalogue}
...
Use `peg_info()` to understand what a dataset contains — its name,
description, category, update frequency, and row count:
```{r peg_info}
peg_info("d4mq-wa44")
```
...
The easiest way is to use `peg_catalogue()` directly in R:
